...
const ImageGenerator: React.FC = () => {
    ...
    // FIX: Changed icon prop from React.ReactElement to React.ElementType and render it directly.
    // This resolves a TypeScript error with cloneElement when adding a className.
    const TabButton: React.FC<{ tabName: 'generate' | 'edit', label: string, icon: React.ElementType }> = ({ tabName, label, icon: Icon }) => (
        <button
            onClick={() => setActiveTab(tabName)}
            className={`flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-t-lg font-semibold transition-colors border-b-2 ${
                activeTab === tabName 
    ...
    return (
        <div className="h-full w-full max-w-5xl mx-auto flex flex-col animate-fadeInUp p-2 sm:p-4">
            <h2 className="text-2xl font-bold text-text-primary text-center flex items-center justify-center gap-3 mb-4">
                <BrushIcon className="h-7 w-7 text-secondary" />
                Design Bay
            </h2>

            <div className="flex border-b border-border">
                <TabButton tabName="generate" label="Generate Image" icon={SparklesIcon} />
                <TabButton tabName="edit" label="Edit Image" icon={EditIcon} />
            </div>

            <div className="flex-1 bg-surface/80 backdrop-blur-sm border border-border border-t-0 rounded-b-xl p-4 sm:p-6">
                {activeTab === 'generate' && <ImageGeneratorTab />}
