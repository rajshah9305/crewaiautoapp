...
const ControlPanel: React.FC<ControlPanelProps> = ({ onSelectTemplate, onLoadPlan, hasSavedPlan, isLoading }) => {
  return (
    <div className="w-full flex flex-col items-start justify-center gap-3 p-4 bg-surface/50 rounded-lg border border-border/30">
        <span className="text-text-secondary font-mono tracking-widest text-sm">&gt; MISSION ARCHIVES:</span>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 w-full">
            {templates.map(template => (
                <button
                    ...
                    disabled={isLoading}
                    className="group bg-surface/70 backdrop-blur-md border border-border/50 p-3 rounded-lg text-text-primary hover:border-primary transition-all duration-300 disabled:opacity-50 flex items-center gap-3 w-full text-left hover:bg-surface-light"
                >
                    {React.cloneElement(template.icon, { className: `h-8 w-8 text-secondary group-hover:text-primary transition-colors duration-300 flex-shrink-0` })}
        ...
        {hasSavedPlan && (
            ...
            >
                ...
                <span className="font-semibold font-sans">Load Saved Mission Plan</span>
            </button>
        )}
    </div>
  );
